...
*   **How it works:** The `rules.json` file contains a JSON list of objects specifying the rule name, pattern string, standard F♭m replacement string, and an optional list of F♭m guard strings. Both pattern and replacement are represented as space-separated F♭m token sequences. At compiler initialization time, the TS `Optimizer` loads and parses this JSON file, splitting the strings by whitespace to construct internal matcher and VM IR structures. If guards are specified, each guard string is evaluated sequentially on a lightweight compile-time F♭m VM instance (`Engine`) to validate constraints. If all return true, the replacement sequence is then executed on the VM to harvest folded instructions.
*   **Rationale:** Provides an exceptionally clean, readable, and self-documenting JSON schema. The rules look exactly like native F♭m stack code, making them easy to write, maintain, and audit, while keeping the tokenizer and binder code in `optimizer.ts` completely trivial.
*   **Alternatives Considered:** Using JSON arrays of individual token strings. Rejected because they are highly verbose and difficult to read and write compared to unified space-separated F♭m strings.
