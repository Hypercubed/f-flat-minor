...
/* 5537376230 */`,Fn=`.import ../lib/prelude.ffp
...
over ++
step
drop drop . /* 837799 */`,Un=`.import ../lib/prelude.ffp

/* http://projecteuler.net/index.php?section=problems&id=16 */

digits: dup [ 10 [ % ] [ / ] bi2 digits ] ? ;

2 1000 ^ digits sum! . /* 1366 */`,$n=`!: dup 1 > [ dup 1 - ! * ] ? trim ;
...
/* Omitted: 1_000_000_000_000 ! … (expected [ 16576 ]) — factorial(10^12) is not practical here. */`,jn=`.import ../lib/prelude.ffp
...
0 1 1 r drop drop ++ . /* 4782 */`,Vn=`.import ../lib/prelude.ffp
...
600851475143 factors last! . /* 6857 */`,Kn=`.import ../lib/prelude.ffp
...
/* 25164150 */`,Zn=`.import ../lib/prelude.ffp
...
99 bottlesOfBeer onTheWall '.' putc`,te=`.import ../lib/prelude.ffp
...
1 fb100 drop`,re=`.import ../lib/prelude.ffp
...
P N nputn
cr
`,oe=`[ 0 120 32 46 32 108 97 118 101 10 59 32 63 32 93 120 91 32 112 117 100 32 99 116 117 112 32 58 120 ]
x: putc dup [x] ? ;
eval . x`,ae=`.import ../lib/prelude.ffp

/* [tutorial](https://code.golf/tutorial) */

'\\0Hello,\\sWorld!\\n' prints

0 [ dup putn cr ++ ] 10 times`,pe=`/* constants 1 2 3 */
true: 1 ; /* true == 1 */
false: 0 ; /* false == 0 */
...
iln: ilb __ln2 * __s / ; /* n iln == floor(ln(n)) */`,be=`.import ./pred.ffp
.import ./arith.ffp
.import ./num.ffp
.import ./atan.ffp
.import ./sqrt.ffp
...
half_pi:
  npi 1 >>
; /* n half_pi == floor(10ⁿ*π/2) */`,we=`.import ../core/core.ff
.import ./arith.ffp
...
nfrac: [ abs ] dip n->S % ; /* x n nfrac == (x - floor(x/10^n))*10^n */`,ye=`.import ../core/core.ff
...
nsqrt:
  ...
  swap n->S2 * isqrt
;
`,Se=`/* Trigonometric words live here. Pi-derived constants are defined in pi.ffp. */
`,Ee=`.import ./core/core.ff
.import ./math/math.ffp
.import ./string/string.ffp
.import ./seq/seq.ffp
.import ./time/time.ffp`,Te=`.import ../core/core.ff
...
`,ze=`.import char.ffp
.import str.ffp`,Ce=`.import ./core/core.ff
.import ./string/string.ffp
...
/* 93326215443944152681699238856266700490715968264381621468592963895217599993229915608941463976156518286253697920827223758251185210916864000000000000000000000000 */`,De=`.import ./lib/prelude.ffp
...
cr putn
0 '\\smoves' println
`;function He(s){const n="../../ff/lib/";if(!s.startsWith(n))throw new Error(`Unexpected library source path: ${s}`);return`/lib/${s.slice(n.length)}`}const Ve=Object.assign({"../../ff/euler/euler1.ffp":Nn,"../../ff/euler/euler10.ffp":Mn,"../../ff/euler/euler13.ffp":Ln,"../../ff/euler/euler14.ffp":Fn,"../../ff/euler/euler16.ffp":Un,"../../ff/euler/euler160.ffp":$n,"../../ff/euler/euler2.ffp":jn,"../../ff/euler/euler20.ffp":Wn,"../../ff/euler/euler25.ffp":Hn,"../../ff/euler/euler3.ffp":Vn,"../../ff/euler/euler4.ffp":Kn,"../../ff/euler/euler46.ffp":Qn,"../../ff/euler/euler48.ffp":Gn,"../../ff/euler/euler5.ffp":Xn,"../../ff/euler/euler6.ffp":Yn,"../../ff/euler/euler7.ffp":Zn,"../../ff/euler/euler8.ffp":Jn,"../../ff/euler/euler9.ffp":ne}),Ke=Object.assign({"../../ff/golf/99bottles.ffp":ee,"../../ff/golf/e-digits.ffp":te,"../../ff/golf/fib.ffp":se,"../../ff/golf/fizzbuzz.ffp":ie,"../../ff/golf/pi-digits.ffp":re,"../../ff/golf/quine.ff":oe,"../../ff/golf/tutorial.ffp":ae});function X(s){const n=s.split("/").pop()??s,e=/^euler(\d+)\.ffp$/.exec(n);return e?[Number(e[1]),n]:[Number.POSITIVE_INFINITY,n]}const Qe=Object.entries(Ve).sort((s,n)=>{const e=X(s[0]),t=X(n[0]);return e[0]!==t[0]?e[0]-t[0]:e[1].localeCompare(t[1])}).map(([s,n])=>({path:Y(s),label:Z(s),source:n}));function Y(s){const n=s.lastIndexOf("/");return`/examples/${n>=0?s.slice(n+1):s}`}function Z(s){const n=s.indexOf("../../ff/");return n>=0?s.slice(n+9):s}const Ge=Object.entries(Ke).map(([s,n])=>({path:Y(s),label:Z(s),source:n})).sort((s,n)=>s.label.localeCompare(n.label)),J=[{path:"/examples/fact.ffp",label:"fact.ffp",source:Re},{path:"/examples/pascal.ffp",label:"pascal.ffp",source:De},{path:"/examples/cbrt.ffp",label:"cbrt.ffp",source:Ne},{path:"/examples/sqrt.ffp",label:"sqrt.ffp",source:Me},{path:"/examples/gcd.ffp",label:"gcd.ffp",source:Le},{path:"/examples/ack.ffp",label:"ack.ffp",source:Fe},{path:"/examples/pi.ffp",label:"pi.ffp",source:Ue},{path:"/examples/catalan.ffp",label:"catalan.ffp",source:$e},{path:"/examples/collatz.ffp",label:"collatz.ffp",source:je},{path:"/examples/hanoi.ffp",label:"hanoi.ffp",source:We},...Qe,...Ge],Xe=Object.fromEntries(Object.entries(Object.assign({"../../ff/lib/core/core.ff":pe,"../../ff/lib/math/ack.ffp":ce,"../../ff/lib/math/arith.ffp":le,"../../ff/lib/math/atan-core.ffp":ue,"../../ff/lib/math/atan.ffp":de,"../../ff/lib/math/cbrt.ffp":fe,"../../ff/lib/math/exp.ffp":he,"../../ff/lib/math/gcd.ffp":_e,"../../ff/lib/math/log.ffp":me,"../../ff/lib/math/math.ffp":be,"../../ff/lib/math/num.ffp":ge,"../../ff/lib/math/pi.ffp":ve,"../../ff/lib/math/precision.ffp":we,"../../ff/lib/math/pred.ffp":ye,"../../ff/lib/math/primes.ffp":xe,"../../ff/lib/math/prn.ffp":qe,"../../ff/lib/math/sqrt.ffp":ke,"../../ff/lib/math/trig.ffp":Se,"../../ff/lib/prelude.ffp":Ee,"../../ff/lib/seq/seq.ffp":Te,"../../ff/lib/string/char.ffp":Ae,"../../ff/lib/string/str.ffp":Pe,"../../ff/lib/string/string.ffp":ze,"../../ff/lib/tap.ffp":Ce,"../../ff/lib/testing.ffp":Ie,"../../ff/lib/time/time.ffp":Be,"../../ff/lib/time/utc.ffp":Oe})).map(([s,n])=>[He(s),n])),Ye=Object.fromEntries(J.map(({path:s,source:n})=>[s,n]));[...J.map(({path:s,label:n})=>`<option value="${s}">${n}</option>`),'<option value="__custom__">Custom</option>'].join(`
`);function Ze(s,n="/main.ffp"){return{[n]:s,...Xe,...Ye}}function P(s){const n=s.startsWith("/"),e=s.split("/").filter(Boolean),t=[];for(const i of e)if(i!=="."){if(i===".."){t.pop();continue}t.push(i)}return`${n?"/":""}${t.join("/")}`||(n?"/":".")}function Je(s){const n=P(s),e=n.lastIndexOf("/");return e<=0?n.startsWith("/")?"/":".":n.slice(0,e)}function nt(...s){const n=s.filter(Boolean).join("/").replaceAll(/\/+/g,"/");return P(n)}function et(s,n){const e=P(s).split("/").filter(Boolean),t=P(n).split("/").filter(Boolean);let i=0;for(;i<e.length&&i<t.length&&e[i]===t[i];)i++;const r=e.length-i,a=t.slice(i);return Array(r).fill("..").concat(a).join("/")||"."}function tt(s){return{readTextFile(n){const e=P(n),t=s[e];if(typeof t!="string")throw new Error(`Virtual file not found: ${e}`);return t},fileExists(n){return typeof s[P(n)]=="string"},path:{isAbsolute(n){return n.startsWith("/")},dirname:Je,relative:et,resolve:nt}}}function st(s={}){const n=new TextDecoder,e=new TextEncoder,t=Array.from(e.encode(s.stdin??""));return{io:{write(i){var r;(r=s.onWrite)==null||r.call(s,n.decode(i))},readByte(){return t.shift()??null}},exit(i){var r;(r=s.onExit)==null||r.call(s,i)},now(){return Date.now()}}}const it="/lib/prelude.ffp";function nn(s,n){const e=console.log;console.log=(...t)=>{s(t.map(String).join(" "))};try{return n()}finally{console.log=e}}async function rt(s,n){const e=console.log;console.log=(...t)=>{s(t.map(String).join(" "))};try{return await n()}finally{console.log=e}}function ot(s,n){let e="";const t=[];let i=0;const r=st({stdin:n,onWrite(m){e+=m},onExit(m){i=m}}),a=new w,c=new V(r),p=new w,l=new V(r),u=Ze(s),_=new q(tt(u),{engine:l,compiler:p},{macroEngineBootstrapFile:it});return{compiler:a,engine:c,preprocessor:_,logs:t,getOutput(){return e},getExitCode(){return i}}}function at(s,n,e){const t=ot(s,n),i=performance.now(),r=nn(c=>t.logs.push(c),()=>{const c=t.preprocessor.preprocess(q.tokenize(s),"/main.ffp");let p=t.compiler.compileToIR(w.tokenize(c),"/main.ffp");const l=t.compiler.validate(p);e&&(p=new Dn().optimize(p));const u=bn(p),_=w.compileToBase64(p);return t.engine.loadIR(p),{preprocessed:c,formattedIr:u,bytecode:_,issues:l}}),a=performance.now();return{preprocessed:r.preprocessed,ir:r.formattedIr,bytecode:r.bytecode,issues:r.issues,compileMs:a-i,execute(){const c=performance.now();nn(l=>t.logs.push(l),()=>{t.engine.run()});const p=performance.now();return{output:t.getOutput(),stack:t.engine.getStack().map(String),logs:[...t.logs],exitCode:t.getExitCode(),executeMs:p-c,cancelled:!1,vmCyclesExecuted:0}},async executeAsync(c={}){const p=performance.now(),l=await rt(_=>t.logs.push(_),async()=>await t.engine.runAsync(c)),u=performance.now();return{output:t.getOutput(),stack:t.engine.getStack().map(String),logs:[...t.logs],exitCode:t.getExitCode(),executeMs:u-p,cancelled:l.cancelled,vmCyclesExecuted:l.vmCyclesExecuted}}}}function O(s){self.postMessage(s)}let M=!1,y=null;function pt(s,n){const e=n.cancelled?"cancelled":"done";return{output:n.output,preprocessed:s.preprocessed,ir:s.ir,bytecode:s.bytecode,issues:s.issues,stack:n.stack,logs:n.logs,exitCode:n.exitCode,compileMs:s.compileMs,executeMs:n.executeMs,terminal:e,vmCyclesExecuted:n.vmCyclesExecuted}}self.onmessage=s=>{const n=s.data;if(n.type==="CANCEL"){y===n.runId&&(M=!0);return}if(n.type!=="RUN")return;const{runId:e,source:t,stdin:i,optimize:r,yieldIntervalMs:a,yieldSliceMax:c}=n;(async()=>{M=!1,y=e;try{const p=at(t,i,r);if(y!==e)return;O({type:"COMPILED",runId:e,compileMs:p.compileMs,preprocessed:p.preprocessed,ir:p.ir,bytecode:p.bytecode});const l=performance.now(),u=await p.executeAsync({yieldIntervalMs:a,yieldSliceMax:c,shouldContinue:()=>!M&&y===e,onChunk:({vmCyclesExecuted:m})=>{y===e&&O({type:"PROGRESS",runId:e,vmCyclesExecuted:m,executeElapsedMs:performance.now()-l})},scheduler:()=>new Promise(m=>{globalThis.setTimeout(m,0)})});if(y!==e)return;const _=pt(p,u);O({type:"RESULT",runId:e,result:_})}catch(p){if(y!==e)return;const l=p instanceof Error?p.message:String(p);O({type:"ERROR",runId:e,message:l})}finally{y===e&&(y=null)}})()}})();
