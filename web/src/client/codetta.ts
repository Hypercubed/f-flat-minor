...
import { syncDetailFooterActiveState, syncSubtabActiveState } from "./detail-tabs.ts";
import {
...
export function mountCodetta(root: HTMLElement, options: CodettaMountOptions = {}) {
  ...
  const detailNav = requireElement<HTMLElement>(root, "#codetta-mode-nav");
  const prevButton = requireElement<HTMLButtonElement>(root, "#codetta-mode-prev");
  const nextButton = requireElement<HTMLButtonElement>(root, "#codetta-mode-next");
  ...
  const detailFooters = Array.from(root.querySelectorAll<HTMLElement>("[data-codetta-detail-footer]"));
  const detailNavigation = options.detailNavigation ?? {
    prevButton,
    nextButton,
    onVisibilityChange: (visible: boolean) => {
      detailNav.hidden = !visible;
    },
  };
  ...
  function setDetailTab(name: string) {
    ...
    syncDetailFooterActiveState(ui.detailFooters, name);
  }
  ...
  function setCodettaRunningState(isRunning: boolean) {
    ...
    ui.runButton.textContent = isRunning ? "Cancel" : "Run";
    ui.runButton.setAttribute("aria-label", isRunning ? "Cancel run" : "Run");
    ui.runButton.classList.toggle("is-cancel", isRunning);
  }
  ...
  function syncDetailNavigation() {
    ...
    detailNavigation.prevButton.disabled = index <= 0;
    detailNavigation.nextButton.disabled = index === -1 || index >= ETUDES.length - 1;
  }

  function setDetailNavigationVisible(visible: boolean) {
    detailNavigation.onVisibilityChange?.(visible);
  }
  ...
  detailNavigation.prevButton.addEventListener("click", () => {
    const index = getEtudeIndex(activeEtude);
    if (index <= 0) {
      return;
    }
  ...
  detailNavigation.nextButton.addEventListener("click", () => {
    const index = getEtudeIndex(activeEtude);
  ...
  let codettaAbort: AbortController | null = null;

  ui.runButton.addEventListener("click", async () => {
    if (codettaAbort !== null) {
      codettaAbort.abort();
      return;
    }

    const abortController = new AbortController();
    codettaAbort = abortController;
    ...
    try {
      ...
      if (run.terminal === "cancelled") {
        latestMatchedOutput = false;
        ui.output.textContent = "(cancelled)";
        ui.result.textContent = "Status: cancelled";
        ui.result.dataset.tone = "neutral";
        syncSubmitState();
        return;
      }
