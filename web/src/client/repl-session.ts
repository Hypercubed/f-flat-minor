...
function withCapturedConsole<T>(collector: (message: string) => void, fn: () => T): T {
...
export class ReplSession {
  ...
  reset() {
    this.output = "";
    this.files = createReplVirtualFiles();
    ...
    this.preprocessor = new Preprocessor(
      createPreprocessHost(this.files),
      {
        engine: macroEngine,
  ...
  inspectValue(valueStr: string): ValueInspection {
    return this.engine.inspectValue(BigInt(valueStr));
  }
