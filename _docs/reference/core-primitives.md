...
| `%`     | `MOD`    | `37`   | `a b % == a%b`                         | Computes the remainder of `a` divided by `b`.\*          |
| `&`     | `AND`    | `38`   | `a b & == a&b`                         | Bitwise AND of `a` and `b`.                              |
...
| `*`     | `MUL`    | `42`   | `a b * == a*b`                       | Multiplies `a` by `b`.                                   |
| `+`     | `ADD`    | `43`   | `a b + == a+b`                       | Adds `a` and `b` together.                               |
| `cons`  | `CONS`   | `44`   | `x y cons == q`                      | Builds an anonymous quotation from two stack values.     |
| `-`     | `SUB`    | `45`   | `a b - == a-b`                       | Subtracts `b` from `a`.                                  |
| `.`     | `DUMP`   | `46`   | `… . == … {prints stack}`            | Prints the entire stack to output.                       |
| `/`     | `DIV`    | `47`   | `a b / == ⌊a/b⌋₀`                       | Divides `a` by `b` using integer division.\*             |
...
| `^`     | `POW`    | `94`   | `a b ^ == a^b`                       | Computes `a` raised to the power of `b`.                 |
| `\|`    | `OR`     | `124`  | `a b \| == a\|b`                     | Bitwise OR of `a` and `b`.                               |
| `~`     | `NOT`    | `126`  | `a ~ == ~a`                          | Bitwise NOT of `a`.                                      |
