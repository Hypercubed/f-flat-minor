...
(define-runtime-path ff-expander-path "../private/expander.rkt")
...
(define (ff-read-syntax path port)
  ...
  (define module-datum
    `(module ff-mod (file ,(path->string ff-expander-path)) ,s-exprs))
  (strip-context (datum->syntax #f module-datum)))

(define (get-info port mod line col pos)
    (lambda (key default)
      (case key
        [(module-language) (path->string ff-expander-path)]
        [else default])))
