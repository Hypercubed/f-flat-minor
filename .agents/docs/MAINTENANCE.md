...
## Repo layout (f-flat-minor)

Beyond `.agents/docs/`, this repository also uses:
...
- Architectural decisions live under `.agents/docs/decisions/` (one file per decision plus `index.md`). Troubleshooting patterns live under `.agents/docs/troubleshooting/` (one file per pattern plus `index.md`). Maintainer **plans** live under `.agents/docs/plans/` (active) and `.agents/docs/plans/archive/` (terminal); see [plans/index.md](plans/index.md), [write-plan CONTRACT](../skills/write-plan/CONTRACT.md), and [plans skill](../skills/plans/SKILL.md). Each **entry** file in `decisions/`, `troubleshooting/`, and `plans/` carries YAML frontmatter so tools can parse metadata without reading the body. Session bundles under `.agents/sessions/` remain temporary evidence, not plans.
...
A lesson belongs in `.agents/AGENTS.md` or root `AGENTS.md` only if it is:
...
Prefer more specific durable homes when possible:

- rationale and architecture tradeoffs: `docs/decisions/`
- recurring failures: `docs/troubleshooting/`
- multi-step procedures: `playbooks/`
- normative constraints: `rules/`
