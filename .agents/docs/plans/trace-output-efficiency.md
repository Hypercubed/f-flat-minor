---
id: trace-output-efficiency
title: "More efficient F-flat-minor trace output"
last_updated: 2026-04-17
description: >
  Reduce the size, parse cost, and memory pressure of recorded VM traces (
  today: verbose JSONL on stderr) while keeping a clear path for tools, th
  e VS Code trace viewer, and optional human/LLM-friendly output.
tags: [debugging, plans]
status: active
kind: initiative
author_kind: ai
prompter: "composer"
...
- `archive/trace-output-human-llm-format.md` established JSONL for agents; this plan **does not** remove that option—it adds **tighter** paths for integrated tools and large traces.
...
- `archive/trace-output-human-llm-format.md` — current JSONL shape and CLI flags.
