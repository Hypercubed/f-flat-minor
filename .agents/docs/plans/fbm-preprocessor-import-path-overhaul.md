---
id: fbm-preprocessor-import-path-overhaul
title: "FBM Preprocessor Import Path Overhaul"
last_updated: 2026-04-11
description: >
  Implemented a two-tier import resolver for the FBM preprocessor: explici
  t file-relative imports keep current behavior, while angle-bracket impor
  ts resolve through a configurable standard-library search path. The ship
  ped shared resolver also includes same-name directory-index r...
tags: [compiler, plans]
status: completed
kind: initiative
author_kind: human
...
2. For each stdlib root in order, try candidates in this order:
   ...
   - if `<root>/<logical-path>` is a directory, apply the existing directory-index proposal from `directory-loading.md` and try `<root>/<logical-path>/<basename>.ffp`, then optionally `.ff`
...
- `directory-loading.md` was folded into this implementation so directory-index resolution shipped once inside the shared resolver.
...
- `directory-loading.md` was folded into this implementation, since both features modified the same path-resolution layer and shipped together.
