---
id: nsqrt-isqrt-plan
title: "`nsqrt` and `isqrt` follow-up plan"
last_updated: 2026-04-22
description: >
  Current integer square root behavior lives in [ff/lib/math/sqrt.ffp](ff/
  lib/math/sqrt.ffp), where [isqrt](ff/lib/math/sqrt.ffp:52) uses the recu
  rsive digit-by-digit helper [__sqrt__next](ff/lib/math/sqrt.ffp:40). Old
  er alternatives [isqrt_newton](ff/lib/math/sqrt.ffp:16) and [...
tags: [math, plans]
status: completed
kind: initiative
---
...
The broader math plan in [`../ffm-math-library.md`](../ffm-math-library.md) currently places sqrt in the separate non-series branch and describes it as Newton-based in the dependency graph section at [`../ffm-math-library.md`]( ../ffm-math-library.md:632 ). That no longer matches the active implementation.
...
Adjust the sqrt notes in [`../ffm-math-library.md`](../ffm-math-library.md) so they no longer imply that the active square-root path is Newton-only.
...
- [x] Update `../ffm-math-library.md` sqrt section so it matches the actual integer-root-first design
