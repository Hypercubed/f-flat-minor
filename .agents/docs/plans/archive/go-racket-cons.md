---
id: go-racket-cons
title: "Add `cons` parity to Racket"
last_updated: 2026-03-22
description: >
  Implement the cons system primitive in the Racket runtime so it matches
  the existing TypeScript behavior and can run ff/lib/seq/*.ffp programs t
  hat depend on cons.
tags: [compiler, plans]
status: completed
kind: initiative
...
## Context
The repo already has a language-level design for `cons` in `../ffm-better-strings.md`, and the TypeScript core already implements `cons` as opcode `,` / ASCII `44`.
