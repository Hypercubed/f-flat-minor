---
id: nexp-implementation
title: "Precision-Driven `nexp` Without Fixed `e`"
last_updated: 2026-03-22
description: >
  Replace the current nexp implementation in ff/lib/math/exp.ffp with a pr
  ecision-driven algorithm that computes floor(10^n * e^x) directly from a
  n integer-only exponential series. The new design removes the fixed __ex
  p__e precision ceiling and makes correctness depend on the ca...
tags: [plans]
status: completed
kind: initiative
author_kind: human
...
## Context
The current implementation derives `nexp` from a baked constant `__exp__e = floor(e * 10^150)`. That makes `nexp` fundamentally limited to about `x * 150` meaningful digits for integer `x`, even if the caller requests a larger `n`. The existing math-library plan in `../ffm-math-library.md` already points toward a two-layer design where internal `-parts` words return rational `p q`, and user-facing words scale once at the end with `p/q*S`.
...
Use stack shapes and queue balancing carefully; keep `p`, `q`, and `t` in separate focused helpers where practical, following the guidance in `../ffm-math-library.md`.
...
- Guidance and recurrence definitions in `../ffm-math-library.md`
