---
id: language-server
title: "F\u266dm Language Server (VSCode Extension)"
last_updated: 2026-04-10
description: >
  Build a future typescript/language-server/ package on top of the already
  -landed shared typescript/compile-service/ foundation, then optionally m
  igrate the existing vscode-f-flat-minor/ extension from direct VS Code p
  roviders and commands to an LSP client for .ff and .ffp files.
tags: [language-server, plans]
status: active
kind: initiative
author_kind: ai
prompter: "anthropic/claude-sonnet-4.6"
---
...
- `web-editor-word-inspector.md` — relevant background for why the shared
  compile/source-map layer exists, but no longer a blocker for Phase 0.
- `static-code-analysis.md` — relevant background for analyzer and
...
- `web-editor-word-inspector.md` — source of `definition-compiler.ts`,
  `DefinitionState`, debounce strategy, and source range mapping design
- `static-code-analysis.md` — source of `analyzer.ts`, `Diagnostic`,
